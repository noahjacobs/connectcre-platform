...
const MAIN_TABS = [
  ...
  ...(process.env.NODE_ENV === 'development' ? [{ id: 'data', label: 'Data' }] : []),
  { id: 'directory', label: 'Directory' },
  { id: 'company', label: 'Company' },
];

type TabType = 'devprojects' | 'tracking' | 'data' | 'directory' | 'company' | null;
...
export default function Header() {
  ...
  const validRoutes = ['/', '/tracking', '/directory', '/data', '/company', '/dashboard', '/admin'];
  ...
  useEffect(() => {
    if (pathname === '/') {
      ...
      // Keep dashboard for backwards compatibility, but default to tracking
      setActiveTab('tracking');
  ...
  return (
    ...
    >
      <div className="px-3 sm:px-6 flex items-center justify-between h-12">
        <div className="relative z-10 flex items-center space-x-3 font-sora group" aria-label="Home page" id="tour-welcome">
          <Link href="/" className="relative">
            <Bolt className="w-6 h-6 text-black dark:text-white group-hover:scale-110 transition-transform duration-200" />
          ...
          <div className={cn(
            "sm:max-w-[80vw] md:max-w-full flex flex-1 justify-center",
            user ? "max-w-[calc(100vw-100px)]" : "max-w-[calc(100vw-150px)]"
          )}>
            <VercelTabs 
              tabs={MAIN_TABS}
              activeTab={activeTab}
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            <Popover open={isHelpPopoverOpen} onOpenChange={setIsHelpPopoverOpen}>
              <PopoverTrigger asChild>
                <Button 
                  className="rounded-full w-full h-full p-[6.5px]"
                  variant="outline"
                  size="icon"
                >
